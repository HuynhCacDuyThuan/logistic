...
const BannerManagement = () => {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://14.225.29.33:8080/api/banner/delete/${id}`, {
        method: "DELETE",
      });
...
const AddBannerForm = ({ fetchBanners }) => {
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    try {
      const response = await fetch("http://14.225.29.33:8080/api/banner/add", {
        method: "POST",
        body: formData,
      });
  ...
  const handleSubmit = async (values, { resetForm }) => {
    ...
    try {
      const response = await fetch(
        `http://14.225.29.33:8080/api/banner/update/${banner.id}`,
        {
          method: "PUT",
          body: formData,
        }
